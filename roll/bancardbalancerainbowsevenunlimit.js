...
function MultiDrawBan(CardToCal, text, type) {
	...
	if (type == 1) 
	{
	cards[0] = rollbase.FunnyDice(17); 
	revs[0] = rollbase.FunnyDice(2);

	for (i = 1; i < 3; i++) {
		for (;;) {
		tmpcard = rollbase.FunnyDice(17);
	...
	if (text == null)
		returnStr = BanCardReply(rollbase.FunnyDice(17)) + ' ' ;
	else
		returnStr = BanCardReply(rollbase.FunnyDice(17)) + ' ' + ' ; ' + text;
	}
...
if (count == 10) returnStr = '撒哈利爾-虹';
if (count == 11) returnStr = '巴哈姆特-虹';
if (count == 12) returnStr = '伊斯拉菲爾-虹';
if (count == 13) returnStr = '宙斯-虹';
if (count == 14) returnStr = '加百列-虹';
if (count == 15) returnStr = '路西法-虹';
if (count == 16) returnStr = '撒旦-虹';
